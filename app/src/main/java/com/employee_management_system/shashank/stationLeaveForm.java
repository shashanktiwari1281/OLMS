...
public class stationLeaveForm extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Task<DocumentSnapshot> t = firebaseFirestore.collection("employees")
                ...
                .get().addOnCompleteListener(task -> {
                    if (Objects.requireNonNull(task.getResult().getLong("Casual Leave")) > 1)
                        leaveAvailable.add("Casual Leave");
                    if (Objects.requireNonNull(task.getResult().getLong("Medical Leave")) > 1)
                        leaveAvailable.add("Medical Leave");
        ...
        leaveSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                startDateTV.setText("");
                endDateTV.setText("");
                if (leaveSpinner.getSelectedItem().toString().equals("Research Leave"))
                    chooseImgBtn.setText("Choose(Required)");
        ...
        endDateTV.setOnClickListener(v -> {
            if (startDateTV.getText().equals("")) {
                ...
                if (!(leaveSpinner.getSelectedItem().equals("Duty Leave") || leaveSpinner.getSelectedItem().equals("Research Leave"))) {
                    ...
                    datePickerDialog2.getDatePicker().setMaxDate(Objects.requireNonNull(getTimeStamp(year + "-12-31T00:00:00Z")).getTime());
        ...
        if (isConnectionAvailable(getApplicationContext()))
            ...
            firebaseFirestore.collection("leaveApplied")
                    ...
                    .addOnCompleteListener(task -> {
                        ...
                        if (!(leaveType.equals("Duty Leave") || leaveType.equals("Research Leave")))
